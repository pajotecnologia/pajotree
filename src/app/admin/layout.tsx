...
export default function SuperAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    { label: "Assinatura & Faturamento", href: "/admin/billing", icon: ReceiptText },
  ];
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex overflow-x-clip font-sans">
        ...
        <div className="min-h-0">
          <div className="h-16 px-4 sm:px-5 flex items-center justify-between border-b border-slate-100">
            <Link href="/admin" className="flex items-center gap-2.5 min-w-0">
              <div className="w-8 h-8 rounded-lg bg-amber-600 flex items-center justify-center shadow-xs shrink-0"><ShieldAlert className="w-4 h-4 text-white" /></div>
              <div className="leading-tight min-w-0"><span className="font-extrabold text-base tracking-tight text-slate-900 block">PAINEL <span className="text-amber-600">MESTRE</span></span><span className="text-[10px] text-amber-700 font-semibold tracking-wider">Super Administrador</span></div>
            </Link>
            <button type="button" aria-label="Fechar menu" onClick={() => setSidebarOpen(false)} className="lg:hidden min-h-11 min-w-11 p-2 rounded-lg text-slate-400 hover:text-slate-700 flex items-center justify-center"><X className="w-5 h-5" /></button>
          </div>
          ...
          <nav className="p-3 space-y-1" aria-label="Navegação administrativa">
            {navItems.map((item) => {
              ...
              return <Link key={item.href} href={item.href} onClick={() => setSidebarOpen(false)} className={`min-h-11 flex items-center gap-3 px-3.5 rounded-xl text-xs font-semibold transition ${active ? "bg-amber-50 text-amber-900 border border-amber-200 shadow-xs" : "text-slate-600 hover:text-slate-900 hover:bg-slate-50"}`}><Icon className="w-4 h-4 shrink-0 text-amber-600" /><span>{item.label}</span></Link>;
            })}
          </nav>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto">
        <header className="min-h-16 px-3 sm:px-6 py-2 border-b border-slate-200 bg-white flex items-center gap-3 shrink-0 sticky top-0 z-30 shadow-2xs">
          <button type="button" aria-label="Abrir menu" onClick={() => setSidebarOpen(true)} className="lg:hidden min-h-11 min-w-11 p-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-600 flex items-center justify-center"><Menu className="w-5 h-5" /></button>
          <span className="min-w-0 truncate text-[10px] sm:text-xs font-bold text-amber-800 uppercase tracking-widest bg-amber-50 border border-amber-200 px-2.5 py-2 rounded-lg">Ambiente de Gestão Global</span>
        </header>
