    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Visualize contatos capturados, mensagens enviadas e converta leads em clientes.
          </p>
        </div>
        ...
        <div className="flex items-center gap-1 bg-white p-1 rounded-xl border border-slate-200 shadow-2xs text-xs font-semibold">
          ...
          >
            Clientes Convertidos ({customers.length})
          </button>
        </div>
      </div>
      ...
      <div className="relative max-w-md">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-xl text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 shadow-2xs"
        />
      </div>
                    ...
                    <tr key={lead.id} className="hover:bg-slate-50/60 transition">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <span className="text-[11px] text-slate-500 block">
                              {lead.email || lead.phone || lead.whatsapp || "Sem contato direto"}
                            </span>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="p-4 max-w-xs">
                        <p className="text-slate-700 truncate text-[11px]">
                          {lead.message || "Nenhuma mensagem preenchida"}
                        </p>
                        <span className="text-[10px] text-slate-400 font-mono">
                          Origem: {lead.source || "Página"}
                        </span>
                      </td>
                        ...
                        <div className="flex items-center justify-end gap-2">
                          ...
                          {lead.status !== "CONVERTED" && (
                            <button
                              ...
                              className="px-3 py-1.5 rounded-xl bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 text-[11px] font-bold transition flex items-center gap-1 shadow-2xs"
                ...
                {filteredCustomers.length > 0 ? (
                  filteredCustomers.map((cust) => (
                    <tr key={cust.id} className="hover:bg-slate-50/60 transition">
                      ...
                      <td className="p-4 text-slate-600">
                        {cust.email || cust.whatsapp || cust.phone || "-"}
                      </td>
                      <td className="p-4 text-slate-500">
                        {cust.company || "Pessoa Física"}
                      </td>
                      <td className="p-4 text-slate-400 text-[11px]">
                        {new Date(cust.createdAt).toLocaleDateString("pt-BR")}
                      </td>
                    </tr>
                  ))
                ) : (
                  <tr>
                    <td colSpan={4} className="p-12 text-center text-slate-400 text-xs">
                      Nenhum cliente cadastrado ainda. Converta seus leads para visualizá-los aqui!
                    </td>
                  </tr>
                )}
              </tbody>
