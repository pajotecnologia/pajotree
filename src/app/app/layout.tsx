  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex overflow-hidden font-sans">
      ...
      >
        ...
        {/* Footer / Plan Banner & Logout */}
        <div className="p-3 border-t border-slate-100 space-y-2 bg-slate-50/50">
          {/* Plan Badge */}
          <Link
            href="/app/billing"
            className="flex items-center justify-between p-2.5 rounded-xl bg-indigo-50/80 border border-indigo-100/80 hover:bg-indigo-100/70 transition group"
          >
            <div className="flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-indigo-600 group-hover:rotate-12 transition" />
              <div>
          ...
          <div className="flex items-center justify-between p-1.5 text-xs">
            <div className="truncate max-w-[150px]">
              <span className="text-slate-800 font-semibold block truncate text-xs">
                {authData?.user?.name}
              </span>
        ...
        <header className="h-16 px-4 sm:px-6 border-b border-slate-200 bg-white flex items-center justify-between shrink-0 sticky top-0 z-30 shadow-2xs">
          ...
          <div className="flex items-center gap-3">
            {authData?.isSuperAdmin && (
              <Link
                href="/admin"
                className="px-3 py-1.5 rounded-xl bg-amber-50 border border-amber-200 text-amber-800 text-xs font-semibold hover:bg-amber-100 transition flex items-center gap-1.5"
              >
                <ShieldAlert className="w-3.5 h-3.5 text-amber-600" />
                <span>Painel Mestre</span>
              </Link>
            )}

            <Link
              href={`/p/${organization?.name?.toLowerCase().replace(/[^a-z0-9]/g, "-") || "minha-empresa"}`}
              target="_blank"
              className="px-3.5 py-1.5 rounded-xl bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 text-indigo-700 text-xs font-semibold transition flex items-center gap-1.5"
            >
