...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!page || page.status !== "PUBLISHED") {
    return {
      title: "Página não encontrada | Pajotree",
    };
  }
  ...
  return {
    ...
    icons: {
      icon: page.settings?.faviconUrl || page.organization.faviconUrl || "/favicon.ico",
    },
  };
}
...
export default async function PublicPage({ params, searchParams }: Props) {
  ...
  // Capturar IP e User-Agent para o PageView
  ...
  // Registrar PageView de forma não-bloqueante
    ...
    <>
        ...
        <script
          dangerouslySetInnerHTML={{
            __html: `
              ...
              fbq('init', '${defaultMetaPixel.pixelId}');
              fbq('track', 'PageView');
            `,
          }}
        />
      ...
      {gaIntegration && (
        <>
          <script
            async
            src={`https://www.googletagmanager.com/gtag/js?id=${gaIntegration.measurementId}`}
          />
          <script
            dangerouslySetInnerHTML={{
              __html: `
      ...
      <PublicPageRenderer page={page as any} />
    </>
  );
}
