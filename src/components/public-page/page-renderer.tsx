...
export function PublicPageRenderer({ page }: PageProps) {
  ...
  const getButtonStyleClass = () => {
    switch (settings.buttonStyle) {
      ...
      case "rounded":
        return "rounded-md";
      case "glass":
        return "rounded-2xl backdrop-blur-md bg-white/10 border border-white/20";
      default:
        return "rounded-2xl";
    }
  };
  ...
  const renderSocialIcon = (iconName: string) => {
    ...
    switch (iconName?.toLowerCase()) {
      ...
      case "linkedin":
      case "share":
        return <Share className={iconClass} />;
      default:
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-md flex justify-end mb-2">
        <button
          ...
          className="p-2.5 rounded-full bg-black/20 hover:bg-black/40 backdrop-blur-md text-white/80 hover:text-white border border-white/10 transition shadow-sm flex items-center gap-1.5 text-xs font-medium"
        >
          {copied ? (
            <>
        ...
        <div className="flex flex-col items-center space-y-3">
          <div className="relative group">
            <div
              ...
              className="p-1 rounded-full shadow-xl transition-transform duration-300 group-hover:scale-105"
            >
              {logoUrl ? (
                <div className="w-24 h-24 rounded-full overflow-hidden bg-slate-900 relative">
                  <Image src={logoUrl} alt={displayName} fill className="object-cover" unoptimized={logoUrl.startsWith("data:")} />
                </div>
              ) : (
                <div className="w-24 h-24 rounded-full bg-slate-900/90 flex items-center justify-center text-2xl font-bold text-white shadow-inner">
                  {initialLetter}
                </div>
              )}
            </div>
            <div className="absolute -bottom-1 -right-1 bg-emerald-500 w-5 h-5 rounded-full border-2 border-slate-900 shadow-md flex items-center justify-center">
          ...
          <div className="space-y-1">
            ...
            {page?.description && (
              <p className="text-sm text-slate-300/90 max-w-xs mx-auto leading-relaxed">{page.description}</p>
            )}
          </div>
        </div>
          ...
          {(page?.links || []).map((link) => {
            ...
            return (
              <a
                ...
                style={{ borderColor: link.featured ? settings.primaryColor : "rgba(255,255,255,0.12)" }}
                className={`w-full group relative flex items-center justify-between p-4 bg-slate-900/70 hover:bg-slate-850 border backdrop-blur-md shadow-lg hover:shadow-2xl transition-all duration-200 transform hover:-translate-y-0.5 ${getButtonStyleClass()} ${
                  link.featured ? "ring-2 ring-indigo-500/40" : ""
                }`}
              >
                {link.featured && (
                  <span
                    style={{ backgroundColor: settings.primaryColor }}
                    className="absolute -top-2.5 left-4 text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded-full text-white shadow-sm flex items-center gap-1"
                  >
                ...
                <div className="flex items-center gap-3.5 text-left">
                  ...
                  <div>
                    <span className="font-semibold text-sm text-white group-hover:text-indigo-300 transition block">
                      {link.title}
                    </span>
                    {link.description && (
                      <span className="text-xs text-slate-400 block mt-0.5">{link.description}</span>
                    )}
                  </div>
                </div>
        ...
        {page.blocks && page.blocks.length > 0 && (
          <div className="w-full space-y-4">
            {page.blocks.map((block) => {
              let config: any = {};
              try {
                config = JSON.parse(block.contentJson);
              } catch {
                ...
                return (
                  <div key={block.id} className="w-full">
                    <PublicContactForm
                      ...
                      textColor="#ffffff"
                      title={block.title || config.title || "Fale Conosco"}
                    />
                  </div>
                );
              }
              if (block.type === "FAQ" && config.items) {
                return (
                  <div
                    key={block.id}
                    className="w-full bg-slate-900/60 border border-white/10 rounded-2xl p-5 backdrop-blur-md text-left space-y-3"
                  >
                    <div className="flex items-center gap-2 mb-2 font-bold text-white text-sm">
                      <HelpCircle className="w-4 h-4 text-indigo-400" />
                      <span>{block.title || "Perguntas Frequentes"}</span>
                    </div>
                    <div className="space-y-2">
                      {config.items.map((item: any, fIdx: number) => (
                        <div key={fIdx} className="border-b border-white/5 pb-2 last:border-0">
                          <button
                            onClick={() => setOpenFaqIndex(openFaqIndex === fIdx ? null : fIdx)}
                            className="w-full flex items-center justify-between text-xs font-semibold text-slate-200 hover:text-white py-1.5 text-left"
                          >
                            <span>{item.question}</span>
                            <ChevronDown
                              className={`w-3.5 h-3.5 transition-transform ${
                                openFaqIndex === fIdx ? "rotate-180 text-indigo-400" : ""
                              }`}
                            />
                          </button>
                          {openFaqIndex === fIdx && (
                            <p className="text-xs text-slate-400 mt-1 pl-1 leading-relaxed">{item.answer}</p>
                          )}
                        </div>
                      ))}
                    </div>
              ...
              if (block.type === "TEXT") {
                return (
                  <div
                    key={block.id}
                    className="w-full p-4 bg-slate-900/40 border border-white/10 rounded-2xl backdrop-blur-md text-left text-xs text-slate-300 leading-relaxed"
                  >
